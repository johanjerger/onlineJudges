...
struct figure
{
  ...
  figure(int i, char t, point pp1, point pp2, point pp3):
    id(i), type(t), p1(pp1), p2(pp2), p3(pp3){};
};
...
bool point_in_triangle(int id, point p, figure t)
{
  bool b1, b2, b3;
  b1 = sign(p, t.p1, t.p2) < 0.0f;
  b2 = sign(p, t.p2, t.p3) < 0.0f;
  b3 = sign(p, t.p3, t.p1) < 0.0f;
  if( (b1 == b2) && (b2 == b3) ) return print(id, t.id);
  else return false;
}
...
int main ()
{
  ...
  while (cin >> type && type != '*')
  {
    if (type == 'r') {
      ...
      cin >> x1 >> y1 >> x2 >> y2 >> x3 >> y3, cin.ignore();
      figures.push_back(figure(++last_id, type, point(x1,y1), point(x2,y2), point(x3,y3)));
    }
  }
  ...
  while(cin >> point_x >> point_y && point_x != 9999.9 && point_y != 9999.9) {
    ...
    for (int i = 0; i < figures.size(); i++) {
      if (figures[i].type == 'r')
        ...
        is_contained |= point_in_triangle(last_id, point(point_x, point_y), figures[i]);
    }
    if (!is_contained)
